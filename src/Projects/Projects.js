...
function Projects() {
    ...
    useEffect(() => {
        const fetchRepos = async () => {
            try {
                const response = await axios.get('https://api.github.com/users/01ethanwright/repos');
    ...
    return (
        <div className='Projects'>
            <div className='repo-list'>
                {repos.map(repo => (
                    <RepoCard key={repo.id} name={repo.name} description={repo.description} languages_url={repo.languages_url} pushed_at={repo.pushed_at} url={repo.html_url} />
                ))}
            </div>
        </div>
    )
